...
class PostDataTable extends BaseDatable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('title', fn (Post $post) => view('admin.posts._tableTitle', compact('post')))
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('title')->title(__('Tên'))->width('18%'),
    ...
    public function printPreview()
    {
        ...
        $source = $this->applyScopes($source);
        $data = $source->get();
        return view('admin.posts.print', compact('data'));
    }
}
